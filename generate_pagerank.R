generate_pagerank = function(subnet, node_ids, progress = NULL){
  ...
  for(i in 1:nrow(selected_nodes)){
    ...
    if(!is.null(progress)){
      progress(detail = paste0("Generating embedding ", i, " out of ", n_mapped, "..."), value = i / n_mapped)
    }
  }
  if(!is.null(progress)){
    progress(detail = paste0("All node embeddings generated!"), value = 1)
  }
